...
If the given File exists (either JSON or YAML), this is read in and used to mask the config object using https://github.com/wankdanker/node-object-mask
...
Pass an object, that is used to mask the config object using https://github.com/wankdanker/node-object-mask
If maskFile is given too, the mask is generated by combining the mask from maskFile with this one.


#### options.maskAllowMode
Type: `Boolean`
Default: false

In deny mode (the default), any property which exists on both the source object and the mask object will NOT be copied; copying only properties which are NOT defined on the mask object.

In allow mode, any property which exists on both the source object and the mask object WILL be copied; copying only properties which ARE defined on the mask object.

This is the `allow` parameter from mask() in node-object-mask.
